...
# Filtr dotyczy TYLKO źródeł z FILTERED_SOURCES (urzędy o mieszanej tematyce).
# SG i UdSC zajmują się migracją z definicji — bierzemy z nich najnowsze wpisy
# bez filtrowania, dzięki czemu nie pokazują się stare newsy.
...
MAX_ITEMS = 12          # ile najnowszych wpisów zapisać (łącznie)
    ...
    for m in re.finditer(pattern, page, re.S):
        ...
        if date_pl:
            try:
                ...
                pass
        # tylko bloki wyglądające jak zajawka artykułu (mają tytuł lub datę),
        # to odfiltrowuje linki nawigacyjne do stron głównych
        looks_like_article = bool(title_m or date_m)
        if looks_like_article and len(title) > 25 and link not in seen \
                and "/aktualnosci" not in link:
            seen.add(link)
